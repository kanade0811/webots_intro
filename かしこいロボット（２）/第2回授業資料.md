...
2. ロボットの位置調整(移動を分かりやすくするため)
    1. `TinkerbotsBase`→`translation`にて、オブジェクトの位置をx=-0.02,y=0.02625,z=-0.04にする
...
これによって、今まで(TimerとCounterで正方形を書いたイマジナリー課題参照)試行錯誤でやるしかなかったことを、回転センサを使い書くことが出来るようになりました。
...
2. ワールドの設定
    1. `TinkerbotsBase`→`floorSize`にて、フィールドの縦横の長さをx=0.6,y=0.4にする。
...
3. ロボット位置の変更
    1. `TinkerbotsBase`→`translation`から、x=0.23,y=0.027,z=-0.04に変更する。
...
4. カラーセンサの追加
    ...
    2. `TinkerbotsBase`→`downSlot TinkerbotsMotor`→`upSlot TinkerbotsColorSensor`にて、`rotation`をangle=1.57、`name`を`3`(もしくは任意の名前)とする。
...
今まで大した説明もせずに使っていた関数ですが、一覧を舌にまとめました。
